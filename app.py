...
rdf_model = pickle.load(open('Models/model_xgb.pkl', 'rb'))
rdt_model = pickle.load(open('Models/model_dt.pkl', 'rb'))
rrf_model = pickle.load(open('Models/model_rf.pkl', 'rb'))
...
if st.button('Analyze Sentiment 🚀'):
    if review_input:
        ...
        prediction = rrf_model.predict(review_features_scaled)
...
if uploaded_file is not None:
    ...
    if 'review' in df.columns:
        ...
        review_features_scaled = scaler.transform(review_features)
        predictions = rrf_model.predict(review_features_scaled)
...
st.markdown("---")
st.write("Made with 💻 by Ayush Singh & Anurag Shukla")
